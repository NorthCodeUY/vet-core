...
const Footer = ({ bgColor }: { bgColor: string }) => {
  return (
    <>
      ...
      */}
      <footer className={`${bgColor} relative z-40 border-t border-vete-primary/20 px-8 md:px-20 py-16 mt-40 shadow-[0_-20px_60px_rgba(0,0,0,0.18)]`}>
        {/* text-white */}
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16 items-start">

          {/* SECCIÓN A: Logo */}
          <div className="space-y-4">
            <div className="flex items-center gap-3 font-extrabold text-2xl text-vete-text-dark">
              <img src="/logo.png" className="w-12 drop-shadow-sm" alt="Logo" />
              {companyInfo.name}<span className="text-vete-primary">.</span>
            </div>
            <p className="text-vete-primary text-base font-medium leading-relaxed">
          ...
          {/* SECCIÓN B: Contacto */}
          <div className="space-y-4">
            <h4 className="text-vete-primary font-bold uppercase tracking-[0.2em] text-sm mb-5 text-center md:text-left">Contacto</h4>
            <ul className="space-y-4 text-vete-text-light text-sm">
              <li className="flex items-center gap-4 hover:text-vete-primary transition-colors duration-300"><Phone size={16} className="text-vete-primary" /> 092 444 510</li>
              <li className="flex items-center gap-4 hover:text-vete-primary transition-colors duration-300"><Mail size={16} className="text-vete-primary" /> contacto@vete.com</li>
            </ul>
          </div>

          {/* SECCIÓN C: Info Extra */}
          <div className="space-y-5 text-right">
            <p className="text-xs font-semibold text-vete-primary uppercase tracking-[0.18em]">Atención 24hs en Salto</p>
            <p className="text-sm text-vete-text-light">© 2024 NorthCode Infrastructure</p>
          </div>
